...
-(void)updateWrappingConstrains
{
    ...
    NSLog(@"height = %lf,weight = %lf", self.frame.size.height,self.frame.size.width);
//    if (self.delegate != nil) {
//        if (self.frame.size.height > 0) {
//            [self.delegate changeTableViewFrameWithHeight:self.frame.size.height];
//        }
//    }
}
...
- (void)removeTagAtIndex:(NSUInteger)index
{
    ...
    [self invalidateIntrinsicContentSize];
    if (self.delegate != nil) {
        [self.delegate deleteContentByTag:index];
    }
}
